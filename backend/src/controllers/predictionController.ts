...
class PredictionController {
  ...
  async getSafetyPrediction(req: Request, res: Response) {
    try {
      ...
      if (!lng || !lat) {
        return res.status(400).json({ success: false, message: 'I need your location to analyze the area' });
      }
      ...
      let level = 'DANGER';
      let recommendation = 'High risk in this area. Move quickly to a Safe Zone or call for help.';
      
      if (safetyScore > 80) {
        level = 'SAFE';
        recommendation = 'Everything looks good. Have a safe journey and stay alert!';
      } else if (safetyScore > 50) {
        level = 'CAUTION';
        recommendation = 'A few incidents reported nearby. Stay on busy, well-lit roads.';
      }
